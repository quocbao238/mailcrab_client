...
class _SettingsDialogState extends State<_SettingsDialog> {
  static const _presetColors = <int>[
    AppSettings.defaultSeedColor, // crab orange
    0xFFD32F2F, // red
    0xFFC2185B, // pink
    0xFF7B1FA2, // purple
    0xFF3949AB, // indigo
  ...
  Future<void> _refreshPermission() async {
    final status =
        await context.read<MailboxBloc>().notifications.checkPermission();
  ...
  Future<void> _requestPermission() async {
    ...
    final status =
        await context.read<MailboxBloc>().notifications.requestPermission();
    if (mounted) {
      setState(() {
        _permission = status;
  ...
  Future<void> _openSystemNotificationSettings() async {
    // Ventura+ notification pane; falls back to the generic settings scheme.
    const candidates = [
  ...
  /// Theme changes apply (and persist) immediately — no Save needed.
  void _applyAppearance() {
    final bloc = context.read<MailboxBloc>();
    bloc.add(MailboxSettingsUpdated(bloc.state.settings.copyWith(
  ...
  Future<void> _testConnection() async {
    ...
    final api = MailCrabApi(_urlController.text);
    try {
  ...
  Widget build(BuildContext context) {
    ...
    final (chipColor, chipLabel) = switch (status) {
      ...
      NotificationPermissionStatus.denied =>
        (theme.colorScheme.error, 'Not granted'),
      NotificationPermissionStatus.unknown =>
        (theme.colorScheme.outline, 'Not required'),
    };
